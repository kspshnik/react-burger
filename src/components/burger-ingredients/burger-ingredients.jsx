  ...
  return (
    <section className={biStyles.section}>
      ...
      <ScrollArea ref={baseRef} contentClass={`${biStyles.scroll} custom-scroll`}>
        ...
        {Object.values(ingredients).filter((el) => el.type === "bun").map((item) => {
       ...
       <ul ref={saucesWatchRef} className={biStyles.grid}>
          {Object.values(ingredients).filter((el) => el.type === "sauce").map((item) => {
        ...
        <ul className={biStyles.grid}>
          {Object.values(ingredients).filter((el) => el.type === "main").map((item) => {
            const { _id, name, price, image } = item;
            const count = order.filter((val) => val === _id).length;
            return (<IngredientCard id={_id}
                                    name={name}
